...
export function handleRSVP(guest, onAccept, onDecline) {
    // Your code here
    if ( guest === undefined || guest === null || typeof onAccept !== "function" || typeof onDecline !== "function") {
        return null;
    }
    if (guest.rsvp === "yes") {
...
export function transformGuestList(guests, ...transformFns) {
    ...
    const result = transformFns.reduce((prev, curr)=> prev === 0 ? curr(guests) : curr(prev) , 0);
    return result;
}
